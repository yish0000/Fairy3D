//------------------------------------------------------------------------
// Name: FlyObb.cpp
// Desc: This file is to implement the class FlyObb.
// ----------------------------------------------------------------------
...
//------------------------------------------------------------------------
void FlyObb::DeTransform(const FlyObb &obb, const FlyMatrix &m)
{
	FlyMatrix mat = m;
	FlyVector vcT;
...
//------------------------------------------------------------------------
void FlyObb::ObbProj(const FlyObb &Obb, const FlyVector &vcV,
...
//------------------------------------------------------------------------
void FlyObb::TriProj(const FlyVector &v0, const FlyVector &v1, 
...
//------------------------------------------------------------------------
bool FlyObb::Intersects(const FlyVector &v0, 
                        const FlyVector &v1, 
                        const FlyVector &v2)
{
	float     fMin0, fMax0, fMin1, fMax1;
	float     fD_C;
	FlyVector vcV, vcTriEdge[3], vcA[3];
...
//------------------------------------------------------------------------
bool FlyObb::Intersects(const FlyRay &Ray, float *t)
{
	float e, f, t1, t2, temp;
	float tmin = -99999.9f, 
	tmax = +99999.9f;

	FlyVector vcP = this->vcCenter - Ray.m_vcOrig;
...
//------------------------------------------------------------------------
bool FlyObb::Intersects(const FlyRay &Ray, float fL, float *t)
{
	float e, f, t1, t2, temp;
	float tmin = -99999.9f;
	float tmax = +99999.9f;
...
//------------------------------------------------------------------------
bool FlyObb::Intersects(const FlyObb &Obb)
{
   float T[3];
   
   // difference vector between both obb
   FlyVector vcD = Obb.vcCenter - this->vcCenter;
...
// ----------------------------------------------------------------------
// Param in -> :  FlyPlane   - array of planes building frustrum
...
//------------------------------------------------------------------------
int FlyObb::Cull(const FlyPlane *pPlanes, int nNumPlanes)
{
	FlyVector vN;
	int       nResult = FLY_VISIBLE;
	float     fRadius, fTest;
