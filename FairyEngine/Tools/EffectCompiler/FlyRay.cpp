//------------------------------------------------------------------------
// Name: FlyRay.cpp
// Desc: This file is to implement the class FlyRay.
// ----------------------------------------------------------------------
...
//------------------------------------------------------------------------
void FlyRay::Set(FlyVector vcOrig, FlyVector vcDir)
{
	m_vcOrig = vcOrig;
	m_vcDir  = vcDir;
}
...
//------------------------------------------------------------------------
void FlyRay::DeTransform(const FlyMatrix &_m)
{
	FlyMatrix mInv;
	FlyMatrix m=_m;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyVector &vc0, const FlyVector &vc1, 
                        const FlyVector &vc2, bool bCull, float *t)
{
	FlyVector pvec, tvec, qvec;

	FlyVector edge1 = vc1 - vc0;
	FlyVector edge2 = vc2 - vc0;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects( const FlyVector &vc0,const FlyVector &vc1, 
                         const FlyVector &vc2,bool bCull,float fL,float *t )
{
	FlyVector pvec, tvec, qvec;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyAabb &aabb, float *t)
{
	bool bInside = true;
	float t0, t1, tmp;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyAabb &aabb, float fL, float *t)
{
	bool bInside = true;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyObb &obb, float *t)
{
	float e, f, t1, t2, temp;
	float tmin = -99999.9f; 
	float tmax = +99999.9f;

	FlyVector vcP = obb.vcCenter - m_vcOrig;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyObb &obb, float fL, float *t)
{
	float e, f, t1, t2, temp;
	float tmin = -99999.9f;
	float tmax = +99999.9f;
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyPlane &plane, bool bCull, 
...
//------------------------------------------------------------------------
bool FlyRay::Intersects(const FlyPlane &plane, bool bCull, float fL,
                        float *t, FlyVector *vcHit)
{
	float Vd = plane.m_vcN * m_vcDir;
