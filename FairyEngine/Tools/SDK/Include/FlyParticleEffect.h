...
public:
    ...
    sController* AddController( const char* cName,int nParIndex,int nEmitterIndex );

    void DeleteParticle( UINT nIndex );
    void DeleteEmitter( UINT nIndex );
    void DeleteController( UINT nIndex );
    ...
    FlyTexture* GetParticleTexture( UINT nIndex );
    FlyTexture* GetParticleTextureByName( const char* cParName );

    FlyParticle* GetParticle( UINT nIndex );
    FlyParticle* GetParticleByName( const char* cName );
    FlyParticleEmitter* GetEmitter( UINT nIndex );
    FlyParticleEmitter* GetEmitterByName( const char* cName );
    sController* GetController( UINT nIndex );
    sController* GetControllerByName( const char* cName );

    UINT GetNumParticles(void);
    UINT GetNumEmitters(void);
    UINT GetNumControllers(void);
