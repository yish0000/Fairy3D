...
FlyRenderEffectCompiler::~FlyRenderEffectCompiler(void)
{
    for( size_t i=0;i<m_Parameters.size();i++ )
    {
        ...
        for( size_t j=0;j<param->annotations.size();j++ )
    ...
    for( size_t i=0;i<m_Techniques.size();i++ )
    {
        ...
        for( size_t j=0;j<tech->annotations.size();j++ )
...
bool FlyRenderEffectCompiler::HandleParameter( FlyXmlReader::XmlNodeAccess param )
{
    ...
    for( size_t s=0;s<param.GetChildCount();s++ )
    {
...
bool FlyRenderEffectCompiler::HandleShader( FlyXmlReader::XmlNodeAccess shader )
{
    ...
    for( size_t f=0;f<shader.GetChildCount();f++ )
    {
...
bool FlyRenderEffectCompiler::HandleTechnique( FlyXmlReader::XmlNodeAccess technique )
{
    ...
    for( size_t p=0;p<technique.GetChildCount();p++ )
    {
        ...
        else if( strcmp(pass.GetName(),"pass") == 0 )
        {
            ...
            for( size_t s=0;s<pass.GetChildCount();s++ )
            {
...
bool FlyRenderEffectCompiler::HandleRenderState( FlyXmlReader::XmlNodeAccess state,FX_Pass* pPass )
{
             ...
             strcmp(state.GetParamValueByName("name"),"tex_alphablend") == 0 )
    {
        ...
        for( size_t i=0;i<state.GetChildCount();i++ )
        {
...
bool FlyRenderEffectCompiler::AddShaderFunc( const char* cFuncName )
{
    ...
    for( size_t i=0;i<m_Shader.funcs.size();i++ )
    {
...
FX_Pass* FlyRenderEffectCompiler::AddPassToTechnique( FX_Technique* pTechnique,const char* cName )
{
    ...
    for( size_t i=0;i<pTechnique->passes.size();i++ )
    {
...
FX_Annotation* FlyRenderEffectCompiler::AddAnnotationToParam( FX_Parameter* param,const char* cName )
{
    ...
    for( size_t i=0;i<param->annotations.size();i++ )
    {
...
FX_Annotation* FlyRenderEffectCompiler::AddAnnotationToTechnique( FX_Technique* technique,const char* cName )
{
    ...
    // Check whether the annotation has been defined.
    for( size_t i=0;i<technique->annotations.size();i++ )
    {
...
FX_Annotation* FlyRenderEffectCompiler::AddAnnotationToPass( FX_Pass* pass,const char* cName )
{
    ...
    // Check whether the annotation has been defined.
    for( size_t i=0;i<pass->annotations.size();i++ )
    {
...
HRESULT FlyRenderEffectCompiler::SaveEffectToFFX( const char* cFfxFile )
{
    ...
    header.num_parameters = (UINT)m_Parameters.size();
    header.num_shaders = (UINT)m_Shader.funcs.size();
    header.num_techniques = (UINT)m_Techniques.size();
...
//------------------------------------------------------------------------
size_t FlyRenderEffectCompiler::GetNumParameters(void) const
{
...
//------------------------------------------------------------------------
size_t FlyRenderEffectCompiler::GetNumShaderFuncs(void) const
{
...
//------------------------------------------------------------------------
size_t FlyRenderEffectCompiler::GetNumTechniques(void) const
{
...
//------------------------------------------------------------------------
const char* FlyRenderEffectCompiler::GetErrorInfo(void) const
{
...
//------------------------------------------------------------------------
FX_Parameter* FlyRenderEffectCompiler::GetParameter( UINT nIndex )
{
...
//------------------------------------------------------------------------
FX_Technique* FlyRenderEffectCompiler::GetTechnique( UINT nIndex )
{
    if( nIndex >= m_Parameters.size() )
        return NULL;
